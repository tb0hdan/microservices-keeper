...
func RunEvents(config *SlackConfiguration) (err2 error) { // nolint
	...
	http.HandleFunc(config.Endpoint, func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			logs.Logger.Printf("body read failed with: %+v\n", err)
		}
		body := buf.String()
		...
		if e != nil {
			logs.Logger.Printf("Verification failed with: %+v\n", e)
		...
		if eventsAPIEvent.Type == slackevents.URLVerification {
			...
			if err != nil {
				logs.Logger.Printf("body write failed with: %+v\n", err)
			}
		}
		if eventsAPIEvent.Type == slackevents.CallbackEvent {
			...
			case *slackevents.AppMentionEvent:
				...
				if err == nil && msg != "" {
					...
					if err != nil {
						logs.Logger.Printf("Could not send message to Slack")
					}

				} else {
					logs.Logger.Printf("Did not send message: %+v, %+v\n", msg, err)
				}
	...
	addr := ":3000"
	logs.Logger.Printf("[INFO] Server listening on addr: %s", addr)
	if err2 = http.ListenAndServe(addr, nil); err2 != nil {
		logs.Logger.Printf("HTTP listener error: %+v\n", err2)
	}
	return err2
}
